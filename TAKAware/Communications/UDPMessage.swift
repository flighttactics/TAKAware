...
class UDPMessage: NSObject, ObservableObject {
    ...
    override init() {
        ...
        do {
            ...
            TAKLogger.error("[UDPMessage]: Could not join group: \(error)")
            return
        }
    ...
    func connect() {
        ...
        guard let connectionGroup = connectionGroup else {
            TAKLogger.debug("[UDPMessage]: Could not start connectionGroup")
            return
        }
